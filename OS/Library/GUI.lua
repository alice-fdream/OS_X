...
local function windowEventHandler(application, window, e1, e2, e3, e4, ...)
	if e1 == "touch" then
		...
		if window ~= window.parent.children[#window.parent.children] then
			...
			Desktop.island.setProcess(window)
...
function GUI.defaultWindow(x, y, width, height)
	local window = Desktop:addChild(GUI.window(x, y, width, height))
	
	window.json = Desktop.json
	window:addChild(GUI.panel(1, 1, width, height, 0x054738))
	
	window:addChild(GUI.text(2, 2, 0xFFFFFF, Desktop.json.main["name"] .. " " .. window.json.main["version"] .. "v"))
	window.actionButton = window:addChild(GUI.roundedButton(width - 3, 1, 3, 3, 0xaa51b0, 0xFFFFFF, 0x880000, 0xFFFFFF, "X"))
	window.actionButton.onTouch = function()
		window:remove()
		Desktop.island.resetProcess()
		coroutine.resume(Desktop.coroutine)
	end
	Desktop.island.setProcess(window)
	return window
end
...
function GUI.titledWindow(x, y, width, height, title, addTitlePanel)
	...
	if addTitlePanel then
		window.titlePanel = window:addChild(GUI.panel(1, 1, width, 1, GUI.WINDOW_TITLE_BACKGROUserializationND_COLOR))
